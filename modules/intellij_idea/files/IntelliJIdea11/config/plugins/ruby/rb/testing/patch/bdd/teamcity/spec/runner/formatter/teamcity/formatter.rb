        ...
        def example_failed_3args(example, failure)
          ...
          if rspec_2?
            ...
            def failure.pending_fixed?
               if defined? RSpec::Core::PendingExampleFixedError
                 # rspec >= 2.0.0.beta.19
...
#        end
        ...
        def close_test_block(example)
          ...
          diagnostic_info = (rspec_2? ? "rspec2 [#{::RSpec::Core::Version::STRING}]" : "rspec1" ) + ", f/s=(#{finished_at_ms}, #{example_data.start_time_in_ms}), duration=#{duration}"
