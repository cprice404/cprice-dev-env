...
require File.dirname(__FILE__) + '/../rails/file_ref_param'
require File.dirname(__FILE__) + '/gem_ref_param'
require File.dirname(__FILE__) + '/group_ref_param'
...
class BundlerParamDefsLoader < BaseParamDefsLoader
  ...
  def registerParamDefs(manager)
    ...
    pdef 'gem', gem_ref, maybe(nil), {:group => [group_ref], :groups => [group_ref], :git => nil,
                                      ...
                                      :platforms => platforms}.merge(git_options)
  ...
  def file_ref(global_only = true)
    ParamDefs::FileRefParam.new(global_only)
  end

  def gem_ref
    ParamDefs::GemRefParam.new
  end

  def group_ref
    either(ParamDefs::GroupRefParam.new, nil)
  end
