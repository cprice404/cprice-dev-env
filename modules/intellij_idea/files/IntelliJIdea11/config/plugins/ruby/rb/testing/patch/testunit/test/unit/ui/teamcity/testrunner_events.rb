      ...
      module TeamCity
        module EventHandlers
          ...
          def suite_finished(suite_name)
            debug_log("Test suit finished: #{suite_name}...")
          ...
          def add_fault(fault)
            ...
            if Rake::TeamCity.is_framework_used(:test_spec) && !(defined? Test::Spec).nil?
              case fault
              when Test::Spec::Disabled, Test::Spec::Empty
                # test_name = fault.short_display
                #  assert_test_valid(test_name) #"short_display" doesn't contains suit name, we cant check test name =/
            ...
            #TODO Due to TeamCity bug - for TeamCity reports stacktrace should include error message
