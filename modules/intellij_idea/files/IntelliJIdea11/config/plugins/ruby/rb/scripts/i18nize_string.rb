...
import org.jetbrains.plugins.haml.HAMLLanguage unless defined? HAMLLanguage
...
module I18nize
  ...
  import org.jetbrains.plugins.haml.psi.HAMLTokenTypes unless defined? HAMLTokenTypes
  ...
  def self.process_haml_code (context, mod, element)
    return if element.nil?
    return unless element.node.element_type == HAMLTokenTypes::TEXT
    # Perform i18n
    ...
    if I18nUtil.isI18nSupportEnabled(mod)
      ...
      unless element.nil?
        ...
        if language == RubyLanguage::INSTANCE
          ...
          # Process HAML Code
        elsif language == HAMLLanguage::INSTANCE
          LOG.debug("Processing HAML code") if LOG.is_debug_enabled
          I18nize.process_haml_code(context, mod, element)
        else
